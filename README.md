...
**Prince Jha Portfolio** is a premium, AI-powered personal developer portfolio built as a full-stack Next.js web application. It goes far beyond a static resume page — featuring an embedded real-time AI Chatbot powered by Google Gemini, a live contact form backed by MongoDB, immersive space-themed canvas animations, scroll-reveal section transitions, animated achievement counters, a VS Code-inspired code mockup, and a fully responsive dark design system. Every section is data-driven and editable from a single source-of-truth data file.
...
- **??? Floating Capability Badges**: Animated orbiting tags (`Full Stack Development`, `DSA`, `Technology`, `Software Engineering`) with per-badge color themes, pulsating glow dots, Lucide vector icons, and independent floating keyframe animations surrounding the profile photo.
...
- **??? Skills Grid with Devicons**: A categorized skill matrix rendering official technology logos sourced from the **Devicons CDN** alongside chip labels, organized across Languages, Frontend, Backend, Databases, Tools, CS Fundamentals, and Data Science.
...
- **?? Backend Status Indicator**: Live backend health check via `/api/status` shown in the navigation bar — green badge when MongoDB and Gemini API are connected, informational otherwise.
...
¦   +-- Header.jsx                    # Sticky nav with live backend status badge
...
- **?? In-Page Resume Viewer**: Embed a PDF.js-powered resume modal for a polished in-app experience instead of linking to Google Drive.
- **?? Blog Section**: Add a markdown-powered or Contentlayer-connected blog to publish technical articles and project deep-dives directly on the portfolio.
- **?? Live GitHub Activity Graph**: Integrate the GitHub Contributions API to display a live commit heatmap and contribution streak in the Achievements section.
- **?? Email Notifications on Contact**: Integrate Nodemailer or Resend API in `/api/contact` to trigger instant email alerts when a visitor submits the form.
- **?? Theme Switcher**: Add alternative color themes (e.g., purple nebula, amber solar, green matrix) toggleable from the header.
- **?? PWA Support**: Add a `manifest.json` and service worker to make the portfolio installable as a Progressive Web App on mobile.
- **?? Voice-Enabled Chatbot**: Extend the AI chatbot with Web Speech API for voice input and text-to-speech output.
- **?? Real-Time GitHub Stats**: Fetch live stats (stars, followers, top repos) from the GitHub REST API and display them as dynamic counters.
- **?? Multi-Language Support (i18n)**: Add `next-intl` for Hindi/English language switching.
- **?? AI Project Recommender**: Use Gemini to analyze visitor interest and recommend the most relevant projects from the portfolio dynamically.
